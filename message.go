...
// Subtypes: "hook_started", "hook_response". Embeds SystemMessage so
// existing SystemMessage checks continue to work.
type HookEventMessage struct {
	SystemMessage
	// HookEventName is the hook event (e.g. "PreToolUse", "PostToolUse", "Stop").
	// Populated from hook_event, hook_name, or hook_event_name in the payload.
	HookEventName string `json:"hook_event_name,omitempty"`
}

func (m *HookEventMessage) messageType() MessageType { return MessageTypeSystem }

// MirrorErrorMessage is synthesized by the SDK when a SessionStore.append call
// fails. It is never emitted by the CLI subprocess directly.
// Subtype: "mirror_error".
type MirrorErrorMessage struct {
	SystemMessage
	MirrorError string `json:"error,omitempty"`
}
...
func ParseMessage(line []byte) (Message, error) {
	...
	case MessageTypeSystem:
		...
		case "hook_started", "hook_response":
			// Populate HookEventName from whichever field the CLI sends.
			hookEventName := m.HookEvent
			if hookEventName == "" {
				hookEventName = m.HookName
			}
			return &HookEventMessage{SystemMessage: m, HookEventName: hookEventName}, nil
		case "mirror_error":
