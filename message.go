...
const (
	...
	MessageTypeMirrorError     MessageType = "mirror_error"
)
...
// HookEventMessage is emitted when hook events are streamed (include_hook_events option).
// Parsed from system messages with subtype "hook_started", "hook_progress", or "hook_response".
...
// MirrorErrorMessage mirrors an error condition from the CLI.
...
func (m *MirrorErrorMessage) messageType() MessageType { return MessageTypeMirrorError }
...
// DeferredToolUse is a tool call that was deferred via the "defer" hook decision.
...
func ParseMessage(line []byte) (Message, error) {
	...
	case MessageTypeSystem:
		...
		// Dispatch task/hook subtypes to dedicated types (matching Python SDK).
		switch m.Subtype {
		case "task_started":
